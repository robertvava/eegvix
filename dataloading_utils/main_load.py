...
def collate_participant_eeg(idx_val, apply_mean = False, all_idx = False, participant = None, data_path ='eeg_dataset/preprocessed', sub = '/sub-02', to_torch = True):
...
class EEGImagePairs(Dataset):
    def __init__(self, X, y, eeg_norm = False, transform = None, apply_mean = True, all_participants = False, test = False):
    ...
    def __getitem__(self, idx):
        if self.eeg_norm:
            eeg = self.standardize_eeg(self.X[idx])
        else: 
            eeg = self.X[idx]
        image_idx = idx

        if self.all_participants: 
            image_idx = image_idx//10
    ...
    def standardize_eeg(self, X):
        eeg_mean = torch.mean(X)
        eeg_std = torch.std(X)
        eeg_data_standardized = (X - eeg_mean) / eeg_std
        return eeg_data_standardized
...
def create_dataloaders(g_cpu, X_train, X_val, X_test, y_train, y_val, y_test, eeg_norm = True, apply_mean = True, all_participants = False, batch_size = 32):
    ...
    train_ds = EEGImagePairs(X_train, y_train, eeg_norm = True, apply_mean= apply_mean, all_participants = all_participants)
    val_ds = EEGImagePairs(X_val, y_val, eeg_norm = True, apply_mean= apply_mean)
    test_ds = EEGImagePairs(X_test, y_test, eeg_norm = True, test = True)

    ### Convert the Datasets to PyTorch's Dataloader format ###
    train_dl = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
    val_dl = DataLoader(val_ds, batch_size=val_ds.__len__(), shuffle=True)
